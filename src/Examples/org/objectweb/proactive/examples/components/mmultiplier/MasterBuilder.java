...
public class MasterBuilder {
			...
			Node node, boolean managed) throws InstantiationException
	{
		...
		itfType = patf.createGCMItfType(
				MMConstants.WORKER_MULTICAST_ITF,
				WorkerMulticast.class.getName(),
				PAGCMTypeFactory.CLIENT,
				PAGCMTypeFactory.MANDATORY,
				PAGCMTypeFactory.MULTICAST_CARDINALITY);
		array.add((PAGCMInterfaceType) itfType);
		...
		ControllerDescription controllerDesc = new ControllerDescription(MMConstants.MASTER_COMP, Constants.PRIMITIVE);
		ContentDescription contentDesc = new ContentDescription(MasterImpl.class.getName());
