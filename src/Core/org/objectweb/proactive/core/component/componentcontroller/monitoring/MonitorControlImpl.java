...
public class MonitorControlImpl extends AbstractPAComponentController implements
	...
	public void addMetric(String name, Metric<?> metric, String compPath) {
		...
		if (!token.hasMoreTokens()) {
			...
			if (debug) System.out.println(hostComponentName + ": addMetric: adding " + name + " locally");
			return;
		}
		...
		if (debug) System.out.println(hostComponentName + ": addMetric: " + nextCompName + " founded");
		String nextPath = "";
		while (token.hasMoreTokens())
			nextPath += token.nextToken() + "/";
		nextCompMon.addMetric(name, metric, nextPath);
	}
	...
	/*
	 * @Override public Object runMetric(String name, Object[] params) { return
	 * metricsStore.calculate(name, params); }
	 */
	...
	/**
	 * Return the metric value of the named metric from the monitor indicated by
	 * the path
	 * 
	 * @param name
